...
hc_map <- read_csv("data/HC_locations.csv",
  ...
  transmute(
    hcode          = normalize_hcode(hcode),
    hc_name        = str_squish(hc_name),
    district       = str_squish(str_remove(as.character(hc_district_unit), "^เขต")),
    district_group = str_squish(as.character(hc_district_group)),
    lat            = as.numeric(str_trim(str_split_fixed(hc_dd, ",", 2)[,1])),
    lon            = as.numeric(str_trim(str_split_fixed(hc_dd, ",", 2)[,2]))
  ) %>%
...
# ตรวจสอบ lat/lon
cat("   Missing lat:", sum(is.na(center_summary$lat)), "\n")
cat("   Missing lon:", sum(is.na(center_summary$lon)), "\n")
cat("   ตัวอย่าง lat/lon:", center_summary$lat[1], center_summary$lon[1], "\n")

output <- list(
  kpi                   = kpi,
  group_summary         = group_summary,
  group3_summary        = group3_summary,
  center_summary        = center_summary,   # มี lat/lon + hc_name แล้ว
...
cat("📋 สรุป KPI:\n")
cat("   - ผู้รับบริการ  :", kpi$total_n, "ราย\n")
cat("   - OOC          :", kpi$ooc_pct, "%\n")
cat("   - ความสม่ำเสมอ :", kpi$continuity_pct, "%\n")
cat("   - ควบคุม BP    :", kpi$bp_pct, "%\n")
cat("   - OR OOC/cont  :", kpi$or_ooc_cont, "(p =", kpi$p_ooc_cont, ")\n")
cat("   - OR OOC/BP    :", kpi$or_ooc_bp,   "(p =", kpi$p_ooc_bp,   ")\n")
cat("   - β  OOC/cost  :", kpi$beta_ooc_cost,"(p =", kpi$p_ooc_cost, ")\n")
cat("🎯 อัปโหลด dashboard_data.json + bma_districts.geojson ขึ้น GitHub ได้เลยค่ะ\n")